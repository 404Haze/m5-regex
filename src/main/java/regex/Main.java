...
public class Main {
    ...
    public static boolean checkForPassword(String str, int minLength) {
        ...
        // ^(?=.*[a-z]) <- checking for lowercases
        // (?=.*[A-Z]) <- checking for uppercases
        // (?=.*\\d) <- check for digit
    ...
    public static List<String> extractEmails(String str) {
        ...
        // [^@\\s]+ <- check local part of any non-@ or non-whitespace chars
        // @(?:...) <- now non capturing group
        // mail\\.utoronto\\.ca|utoronto\\.ca <- utoronto emails
    ...
    public static boolean checkForDoubles(String str) {
        if (str == null) return false;
        // .*([A-Z]) <- get capital letters
        // .*\\1 <- then match same gotten letter
        // .* <- and the rest
